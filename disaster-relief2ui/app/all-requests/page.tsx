  ...
  const applyToRequest = async (requestId: number) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/volunteer/apply/${requestId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="text-center space-y-6 pt-4">
            <div>
              ...
              <p className="text-text-secondary text-lg font-body max-w-2xl mx-auto">
                Discover requests from your community and offer your
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <Select value={urgencyFilter} onValueChange={setUrgencyFilter}>
                  ...
                  <SelectContent className="bg-glass-02 border-glass-border text-text-primary font-mono text-sm uppercase">
                    <SelectItem
                      value="all"
                      className="focus:bg-glass-01 focus:text-accent-teal"
                    >
                      🎯 All Threat Levels
                    </SelectItem>
                    <SelectItem
                      value="high"
                      className="focus:bg-glass-01 focus:text-accent-red font-bold"
                    >
                      🔴 Critical Priority
                    </SelectItem>
                    <SelectItem
                      value="medium"
                      className="focus:bg-glass-01 focus:text-accent-amber"
                    >
                      🟡 Medium Priority
                    </SelectItem>
                    <SelectItem
            ...
            <>
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {filteredRequests.map((request) => (
                  <Card
                    key={request.id}
                    className="glass-card-elevated hover:bg-glass-02 transition-all duration-300 group flex flex-col h-full border-t-4 border-t-transparent hover:border-t-accent-teal"
                  >
                    <CardContent className="p-6 flex flex-col h-full space-y-4">
                      ...
                      <div className="flex items-start justify-between min-h-[4rem]">
                        <div className="flex-1 min-w-0 pr-2">
                          ...
                          <Badge
                            className={`uppercase tracking-label font-mono bg-transparent border flex flex-row items-center justify-center gap-1 w-fit ${
                      ...
                      {user?.role === "volunteer" && request.user && (
                        <div className="bg-glass-01 border border-glass-border rounded-lg p-4 space-y-2 mt-2">
                          <p className="text-xs uppercase tracking-label font-bold text-accent-teal">
                            Origin Contact Data
                          </p>
                          <div className="space-y-1.5 text-xs font-mono text-text-secondary">
                            ...
                            <p className="flex items-center gap-2 truncate">
                              <span className="w-3 h-3 text-text-muted flex items-center justify-center">
                                @
                              </span>
                              {request.user.email}
                            </p>
                        ...
                        {user?.role === "volunteer" &&
                          user.id !== request.user_id && (
                            <Button
                              ...
                              disabled={
                                applyingTo === request.id || request.has_applied
                              }
                              className={`flex-1 h-12 font-display font-bold transition-all duration-300 ${
                                request.has_applied
