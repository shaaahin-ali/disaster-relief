...
export function Header({ onAuthClick }: HeaderProps) {
	...
	return (
		<header
			className={cn('fixed top-0 z-50 w-full border-b border-transparent transition-all duration-500', {
				'bg-black/80 supports-[backdrop-filter]:bg-black/50 border-white/10 backdrop-blur-md':
					scrolled,
			})}
		>
			<nav className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-6">
				...
				<div className="hidden items-center gap-6 md:flex">
					<div className="flex items-center gap-4 hidden">
						{links.map((link) => (
							<a key={link.label} className={cn(buttonVariants({ variant: 'ghost' }), "text-white/70 hover:text-white")} href={link.href}>
								{link.label}
							</a>
						))}
					</div>
					...
					<button 
						className="text-xs font-mono uppercase tracking-widest px-5 py-2.5 border border-white/20 hover:border-white/60 hover:bg-white hover:text-black transition-all duration-300 rounded-none"
			...
			<MobileMenu open={open} className="flex flex-col justify-between gap-2 bg-black border-white/10">
				<div className="flex flex-col gap-4 mt-8">
					<button 
						className="w-full text-left font-mono uppercase tracking-widest text-white/50 hover:text-white py-4 transition-colors"
						onClick={() => { setOpen(false); onAuthClick?.("signin"); }}
					>
						Sign In
					</button>
					<button 
						className="w-full text-center font-mono uppercase tracking-widest px-5 py-4 border border-white/20 hover:bg-white hover:text-black transition-all duration-300"
						onClick={() => { setOpen(false); onAuthClick?.("signup"); }}
					>
						Get Started
					</button>
				</div>
